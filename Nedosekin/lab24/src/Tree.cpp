...
void ExpressionTree::lab_task(Node *node)
{
    ...
    if (node->get_left() && (node->get_left()->get_token() == '*')) {
        if ((node->get_left()->get_left() && node->get_left()->get_left()->get_token() == '0') || (node->get_left()->get_right() && node->get_left()->get_right()->get_token() == '0')) {
            delete_branch(node->get_left());
            node->set_left(nullptr);
        }
    }
    if (node->get_right() && node->get_right()->get_token() == '*') {
        if ((node->get_right()->get_left() && node->get_right()->get_left()->get_token() == '0') || (node->get_right()->get_right() && node->get_right()->get_right()->get_token() == '0')) {
            delete_branch(node->get_right());
